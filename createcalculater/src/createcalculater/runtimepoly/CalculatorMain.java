package createcalculater.runtimepoly;

import java.util.Arrays;
import java.util.Objects;
import java.util.Scanner;

import createcalculater.exception.EquationInCompleteException;
import createcalculater.exception.InvalidOperatorException;
import createcalculater.exception.InvalidValueException;
...
//		String expression = "sin(90)+cos(90)*son(45)";
		System.out.println(expression);
		try {
			if (expression.endsWith("+") || expression.endsWith("-") || expression.endsWith("*")
					|| expression.endsWith("/")) {

				throw new EquationInCompleteException("pls do not expression ends with Operators(+, -, *, /) ");
			}
			...
			for (int i = 0; i < split.length; i++) {
				...
				if (!(s.matches("[0-9]+"))) {
					...
					TrignometircOp trignometircOp = Arrays.stream(TrignometircOp.values())
					...
					if (Objects.nonNull(trignometircOp)) {
						...
						ScintificOp scintificOp = Arrays.stream(ScintificOp.values())
								.filter(val -> val.name().equals(methodName)).findFirst().orElse(null);
	...
	public static double applyTrignometricOperation(String operation, Double value) throws InvalidOperatorException {
		TrignometircOp key = TrignometircOp.valueOf(operation);
		double result;
		switch (key) {
		case sin: {
	...
	public static double applySciOperation(String operation, Double value) throws InvalidOperatorException {
		ScintificOp key = ScintificOp.valueOf(operation);
		double result;
		switch (key) {
		case abs: {
